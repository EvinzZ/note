...
共同点：
1. 都是基于Lucene搜索服务器基础之上开发，高性能的企业级搜索服务器。
2. 他们都是基于分词技术构建的倒排索引的方式进行查询

不同点：
1. 当实时建立索引的时候，solr会产生io阻塞，而es则不会，es查询性能要高于solr。
2. 在不断动态添加数据的时候，solr的检索效率会变的低，es没有改变
3. solr利用zookeeper进行分布式管理，es自身带有分布式系统管理功能。
4. solr需要部署到web服务器上，例tomcat，启动tomcat的时候需要配置tomcat与solr的关联。（solr的本质是一个动态web项目）
5. solr支持更多的格式数据[xml, json, csv等]，es只支持json文件格式
6. solr是传统搜索应用的有力解决方案，es更适用于新兴的实时搜索应用。
    - 单纯的对已有数据进行检索，solr效率更好
7. solr官网提供的功能更多，es更注重与核心功能，高级功能多有第三方插件。